...
import { UserJwt, RedisCacheService } from '@/shared';
  ...
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  private async emitStatusToFriends(activeUser: ActiveUser) {
    ...
    for (const f of friends) {
      ...
      // Notifies the friend via their socket connection (friend.socketId) that this user (activeUser) is now online or offline.
      this.server.to(friend.socketId).emit('friendActive', {
        id: activeUser.id,
        isActive: activeUser.isActive,
      });

      // If the user is now active, send the friend’s current status back to the user, so the user sees their friend as online/offline in real-time.
